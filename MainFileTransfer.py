...
def mainDocFileTransfer(file_names,foldername,targetfolder,downloads):
    ...
    if os.path.exists(mainfolder)==False:
        os.mkdir(mainfolder)
    if os.path.exists(os.path.join(mainfolder,"Master_Agreements"))==False:
        os.mkdir(os.path.join(mainfolder,"Master_Agreements"))

    '''______Create Versioning of the dupliacte files________'''
    extenChange=lambda x:x.replace(".docx","").replace(".txt","").replace(".csv","").replace(".pdf","").replace(".xlsx","").replace(".py","").replace(".docs","").replace(".doc","").replace(".html","").replace(".json","").replace("(","").replace(")","").replace(".xlsm","").replace(".yaml","").replace(".csv","").replace(".csv","").replace(".PDF","")    
    ...
    for filename in Allfiles:
        for dirfile in fileNamesinDownloads:
            if filename in dirfile:
                if os.path.exists(os.path.join(downloads, dirfile)):
                    try:
                        ...
                        MainFileslist.append(dirfile+" -error : "+str(e))
